...
import { bookingNavForRole, isBookingPath } from "@/lib/admin/nav";
import { cn } from "@/lib/utils";
import type { AdminRole } from "@/stores/admin-store";

export function AdminBookingNav({
  current,
  role,
  copy,
  onGo,
  ...
  const inBookings = isBookingPath(current);
  const [open, setOpen] = useState(inBookings);
  const children = bookingNavForRole(role);

  useEffect(() => {
    if (inBookings) setOpen(true);
  }, [inBookings]);
  ...
  return (
    <div>
      <button
        ...
        aria-expanded={open}
      >
        <span>{copy.nav["/admin/bookings"]}</span>
          ...
          {children.map((item) => {
            const active =
              item.href === "/admin/orders"
                ? current === item.href || current.startsWith("/admin/orders/")
                : current === item.href;
            return (
              <button
                key={item.href}
