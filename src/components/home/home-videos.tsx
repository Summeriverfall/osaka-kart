...
export function HomeVideos({ limit }: { limit?: number }) {
  ...
  return (
    <section id="videos" className="bg-[#F5F5F7] py-20">
      <div className="mx-auto max-w-6xl px-4">
        <h2 className="mb-3 text-3xl font-semibold md:text-4xl">{title}</h2>
        <p className="mb-10 text-[#6E6E73]">{lead}</p>

        {featured ? (
          <div className="mb-10 overflow-hidden rounded-2xl border border-[#ECECEF] bg-white shadow-[0_8px_28px_rgb(0_0_0_/_5%)]">
            ...
            <div className="flex items-center justify-between gap-3 px-5 py-4">
              ...
              {sourceLabel(featured) ? (
                <span className="rounded-full bg-[#EEF3F8] px-3 py-1 text-xs font-semibold text-[#1B365D]">
                  {sourceLabel(featured)}
                </span>
              ) : null}
            </div>
          ...
          {rest.map((clip) => {
            ...
            return (
              <button
                key={clip.id}
                type="button"
                className="overflow-hidden rounded-2xl border border-[#ECECEF] bg-white text-left shadow-[0_8px_28px_rgb(0_0_0_/_5%)] transition hover:-translate-y-0.5 hover:shadow-[0_12px_36px_rgb(0_0_0_/_8%)]"
              ...
              >
                ...
                <div className="p-4">
                  ...
                  {platform ? (
                    <p className="mt-1 text-xs font-semibold text-[#1B365D]">
                      {t("watchOn", { platform })}
                    </p>
                  ) : null}
                </div>
      ...
      >
        {active && resolved ? (
          <div className="aspect-video overflow-hidden rounded-xl border border-[#ECECEF] bg-black">
