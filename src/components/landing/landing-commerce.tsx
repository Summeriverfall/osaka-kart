...
export function ClickToPlayClip({
  video,
  fallback,
  startAt = 0,
  label,
...
export function LandingCommerce({ plans, addons, locale, theme, copy }: CommerceProps) {
  return (
    <>
      <LandingBook plans={plans} addons={addons} locale={locale} theme={theme} />
...
export function LandingBook({
  ...
  const copyCol = (
    <div className="shop-book-copy">
      {theme === "acid" ? null : <p className="shop-kicker">03</p>}
  ...
  const bookGrid = cn("shop-wrap shop-book-grid", theme === "acid" && "is-swap");

  return (
    <section id="book" className={cn("shop-book", theme === "acid" && "acid-book")}>
      {step === 1 ? (
        ...
        <form className={bookGrid} onSubmit={onSubmit}>
          {copyCol}
          <div className="shop-cal-card">
...
export function LandingFlow({ copy, theme }: { copy: LandingCopy; theme: SiteTheme }) {
  if (theme === "acid") {
    return (
      <section className="acid-flow-band" id="flow">
        <div className="acid-flow-inner">
...
export function LandingReviews({ copy, theme }: { copy: LandingCopy; theme: SiteTheme }) {
  ...
  if (theme === "acid") {
    return (
      <section id="reviews" className="acid-section acid-reviews">
        <h2 className="acid-h2">{title}</h2>
        <p className="acid-lead">{lead}</p>
        <div className="acid-stickers">
          {reviews.map((item, index) => (
            <blockquote key={item.id} className={`rot-${index % 3}`}>
...
export function LandingFaq({ copy, theme }: { copy: LandingCopy; theme: SiteTheme }) {
  const locale = useLocale();
  const cms = useLiveCms();
  const shop = useTranslations("Shop");
  ...
  if (theme === "acid") {
    return (
      <section id="faq" className="acid-faq-band">
        <div className="acid-band-inner acid-faq-inner">
          <div className="acid-faq-stamp" aria-hidden>
            FAQ
          </div>
...
export function LandingExperience({ theme }: { theme: SiteTheme }) {
  const locale = useLocale();
  const cms = useLiveCms();
  ...
  const cards = (
    <div className={theme === "oni" ? "oni-clip shop-xp" : theme === "acid" ? "acid-reel-track" : "shop-xp"}>
  ...
  if (theme === "acid") {
    return (
      <section id="experience" className="acid-reel">
        <div className="acid-band-inner">
          <div className="acid-reel-head">
            <h2 className="acid-h2">{title}</h2>
            <p className="acid-lead">{lead}</p>
          </div>
          {cards}
...
export function LandingNotes({ theme }: { theme: SiteTheme }) {
  const locale = useLocale();
  const cms = useLiveCms();
  ...
  if (theme === "acid") {
    return (
      <section id="press" className="acid-press-rail">
        <div className="acid-band-inner">
          <h2 className="acid-h2">{title}</h2>
          <div className="acid-press-track">
            {cards.map((item) => {
              const body = (
                <>
...
export function LandingFeatures({ copy, theme }: { copy: LandingCopy; theme: SiteTheme }) {
  if (theme === "acid") {
    return (
      <section id="safety" className="acid-section acid-poster-band">
...
export function LandingVisit({ copy, theme }: { copy: LandingCopy; theme: SiteTheme }) {
  const locale = useLocale();
  const cms = useLiveCms();
  ...
  if (theme === "acid") {
    return (
      <section id="access" className="acid-meet">
        <div className="acid-meet-sign">
          <p className="acid-meet-kicker">{title}</p>
          <h2>{walk}</h2>
          <p className="acid-meet-lead">{lead}</p>
          {maps ? (
            <a className="acid-meet-addr" href={maps} target="_blank" rel="noreferrer">
              {address}
            </a>
          ) : (
            <p className="acid-meet-addr">{address}</p>
          )}
          <p className="acid-meet-hours">{contact("hours", { hours })}</p>
        </div>
        <div className="acid-meet-channels">
          <h3>{howTitle}</h3>
          <BookingChannels stacked={false} />
        </div>
      </section>
    );
  }
