  ...
  return (
    <>
      ...
      <p className="mt-3 text-xs text-slate-400">
        {typeof limit === "number"
          ? `最近 ${rows.length} 条${filtered.length > rows.length ? `，筛选后共 ${filtered.length} 条` : ""}`
          : `共 ${rows.length} 条`}
      </p>
        ...
        {rows.map((item) => (
          <article key={item.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="shrink-0 rounded-full bg-white px-2 py-0.5 text-[11px] text-slate-600">{item.type}</span>
            </div>
            <p className="mt-1 text-sm font-semibold">
              {item.actor}
              <span className="ml-1 font-normal text-slate-500">· {item.role}</span>
            </p>
      ...
      {rows.length === 0 ? <p className="px-4 py-8 text-center text-sm text-slate-400">没有符合条件的日志</p> : null}
    </>
  );
}
