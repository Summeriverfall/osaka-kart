...
import { isSettingsPath, settingsNavForRole } from "@/lib/admin/nav";
...
export function AdminSettingsNav({
  ...
  const inSettings = isSettingsPath(current);
  const [open, setOpen] = useState(inSettings);
  const children = settingsNavForRole(role);
  ...
  return (
    <div>
      <button
        ...
        aria-expanded={open}
      >
        <span>{copy.nav["/admin/settings"]}</span>
